      ...
      <div className="max-w-md mx-auto">
            ...
            <div key={product.id} className="bg-white rounded-lg shadow-md p-4 flex items-center justify-between min-h-[6rem]">
              <div className="flex items-center space-x-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600">{product.co2PerKg} Co2/kg</p>
                  <p className="text-lg font-bold text-gray-900 mt-1">${product.price}</p>
                  <p className="text-sm text-green-700 font-semibold">Points: {product.points}</p>
                </div>
              </div>
              <button
        ...
        <div className="bg-white rounded-lg shadow-md p-4 mt-6">
          ...
          {cart.length === 0 ? (
            ...
            <div className="space-y-2">
              {cart.map((item) => (
                <div key={item.id} className="flex justify-between items-center bg-gray-100 p-3 rounded-md">
                  <div>
                    <h3 className="font-medium text-gray-800">{item.name}</h3>
                    <p className="text-sm text-gray-600">${item.price}</p>
                    <p className="text-sm text-green-700">Points: {item.points}</p>
                  </div>
          ...
          {showDiscount && (
            <div className="mt-4 p-2 bg-green-100 text-green-800 rounded-md text-center transition-opacity duration-500">
              ...
              <p className="text-gray-600 line-through">Total: ${cart.reduce((acc, item) => acc + item.price, 0)}</p>
              <p className="text-2xl font-bold text-green-600">New Total: ${discountedTotal}</p>
              <button onClick={usePoints} className="bg-green-500 text-white px-4 py-2 rounded-md mt-1">Use Points</button>
            </div>
          )}
