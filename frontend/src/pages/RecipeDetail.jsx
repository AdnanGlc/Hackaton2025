    ...
    const fetchRecipeDetail = async () => {
      try {
        const response = await fetch(
          `https://iotimages-f9fegmephhc5c8e7.canadacentral-01.azurewebsites.net/api/RecipeGetById?request=${id}`,
          {
            method: "GET",
        ...
        <>
          ...
          <div className="flex flex-col items-start px-6 mt-[-20px] w-full rounded-t-4xl bg-white pt-[20px]">
            ...
            <div className="flex justify-between mt-5 text-sm tracking-wide w-full">
              <div className="uppercase text-neutral-700 font-semibold">
                Ingredients
              </div>
            </div>
            ...
            <ul className="grid grid-cols-2 w-full gap-2 mt-3">
              {recipe.products && recipe.products.length > 0 ? (
                recipe.products.map((product, index) => (
                  <li
                    key={index}
                    className="bg-orange-100 border border-orange-500 rounded-xl flex items-center justify-center p-3"
                  >
